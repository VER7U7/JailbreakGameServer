...
public class NetworkEngine extends Thread {
    ...
    @Override
    public void run() {
        ByteBuffer receiveBuffer = ByteBuffer.allocate(NEJB_PROTOCOL_MAX_LENGTH);
    ...
    public void handleHandshake(SocketAddress socketAddress, ByteBuffer buffer) throws IOException {
        ...
        if (!nonAuthedConnections.containsKey(socketAddress)) {
            if (packet.getPacketId() == INCOMING_NEW_CONNECTION_PACKET) {
                int clientVersion = ByteBuffer.wrap(packet.getData()).getInt();
                ...
                if (clientVersion == SERVER_VERSION) {
                    ...
                    channel.send(ByteBuffer.wrap(outPacket.getFormattedData()), socketAddress);
                } else {
    ...
    private NetworkPacket parsePacket(ByteBuffer buffer) throws IllegalPacketFormatException {
        ...
        short dataLength = buffer.get(headerBuffer, 0, 4).getShort(4);
        if (!Arrays.equals(headerBuffer, NEJB_PROTOCOL_HEADER))
            throw new IllegalPacketFormatException("Protocol header does not match.");

        short packetID = buffer.getShort(6);
        short playerID = buffer.getShort(8); //for not logined users is 0
        long timestamp = buffer.getLong(10);
        byte[] data = new byte[dataLength];
        buffer.get(data, 18, dataLength);
        long commitHash = buffer.getLong(18 + dataLength);
    ...
    private void processOutgoingMessages() {
        ...
        while ((outgoingMessage = outgoingQueue.poll()) != null) {
            ...
            if (session != null) {
                ByteBuffer sendBuffer = ByteBuffer.wrap(outgoingMessage.getPacket().getFormattedData());
                try {
